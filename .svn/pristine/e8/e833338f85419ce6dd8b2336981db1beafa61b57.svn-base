...
- (void)pulishShowChanceWithTitle:(NSString *)title showTime:(NSString *)showTime dirt:(NSString *)dirt style:(NSString *)style contentext:(NSString *)contentext utoken:(NSString *)utoken isHub:(BOOL)isHub success:(void (^)(AFHTTPRequestOperation *, id))success failure:(void (^)(AFHTTPRequestOperation *, NSError *))failure
{
    [self postWithPath:[self getUrlWithPath:@"add"] parameters:@{@"title":title,@"showTime":showTime,@"dirt":dirt,@"style":style, @"contentext":contentext,@"utoken":utoken} isHub:isHub success:success failure:failure];
}
