...
- (instancetype)init
{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(actionBtnLeft) name:@"actionBtnLeft" object:nil];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(actionBtnRight) name:@"actionBtnRight" object:nil];
...
-(void)actionBtnLeft{
    int page = (self.menuMonthsView.contentOffset.x - self.menuMonthsView.frame.size.width/2) / self.menuMonthsView.frame.size.width;
    [self.menuMonthsView setContentOffset:CGPointMake((page)*self.menuMonthsView.frame.size.width, 0) animated:YES];

}

-(void)actionBtnRight{
    int page = (self.menuMonthsView.contentOffset.x - self.menuMonthsView.frame.size.width/2) / self.menuMonthsView.frame.size.width;
    [self.menuMonthsView setContentOffset:CGPointMake((page+2)*self.menuMonthsView.frame.size.width, 0) animated:YES];
