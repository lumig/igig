...
{
    if (indexPath.row == 0) {
        if([self.showTime compare:self.date] == NSOrderedAscending){
            [MBProgressHUD showWithMessage:@"过期无法发布演出机会"];
            return;
        }
        StateShowViewController *stateVC = [[StateShowViewController alloc] init];
        stateVC.showTime = self.showTime;
        stateVC.enable = YES;
        [self.navigationController pushViewController:stateVC animated:YES];
    }else
    {
        ShowDeatilEditViewController *editVC = [[ShowDeatilEditViewController alloc]init];
        editVC.showTime = self.showTime;
