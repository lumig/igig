...
- (void)layoutSubviews
{
    ...
    for(UIView *view in self.subviews){
        view.frame = CGRectMake(x, -4, width, height);
...
#warning 为日历添加演出数字
        //为日历添加演出数字
        NSString  *showStr = [[DatelistTool shardInstance]showNumWithData:currentDate];
        
        NSString *chanceStr = [[DatelistTool shardInstance]showchargeNumWithData:currentDate];

        [view setShow:showStr AndChang:chanceStr];
