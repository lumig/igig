...
static const CGFloat kBrowserImageViewWidth = 375;
...
- (void)layoutSubviews
{
    ...
    if (self.image.size.height > self.bounds.size.height) {
        ...
        _scrollImageView.bounds = CGRectMake(0, 0, kBrowserImageViewWidth, self.image.size.height/2.0f);
        _scrollImageView.center = CGPointMake(_scroll.frame.size.width * 0.5, _scrollImageView.frame.size.height * 0.5);
        _scroll.contentSize = CGSizeMake(0, self.image.size.height/2.0f);
...
{
//    if ((_totalScale < 0.5 && totalScale < _totalScale) || (_totalScale > 1.5 && totalScale > _totalScale)) return; // 最大缩放 2倍,最小0.5倍
//    
